...
| 512 | Casts a literal value or variable to bool. Non-Zero Ints and Floats become `true`, Arrays become `true` if `null` and Strings are always coerced to `false`. <br>```$local1 = (bool)$local2;``` |
